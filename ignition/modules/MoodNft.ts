...
export default buildModule("MoodNftModule", (m) => {
  const happySvgUri = m.getParameter(
    "happySvgUri",
  ...
  const sadSvgUri = m.getParameter(
    "sadSvgUri",
  ...
  const moodNft = m.contract("MoodNft", [happySvgUri, sadSvgUri]);

  m.call(moodNft, "mintNft", []);
  return { moodNft };
});
